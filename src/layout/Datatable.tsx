...
interface DataTableProps {
    ...
    object: string;
    onEdit: (arg0: Record<string, unknown>) => void;
}

export default function DataTable({ columns, data, object, onEdit }: DataTableProps) {
    ...
    async function handleStatus(event: React.MouseEvent<HTMLDivElement>) {
        ...
        const record = tableData[Number(index)];
        if (!record) return;

        const recordToUpdate = { isActive: !record.isActive, id: record.id }
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        const [_, error] = await updateRecord({ record: recordToUpdate, object: object });

        if (error) { console.log('Error:', error) }

        setTableData((prev) => {
            const updatedData = [...prev];
            updatedData[Number(index)] = { ...updatedData[Number(index)], isActive: !record.isActive };
            return updatedData;
        });
    }
    ...
    async function handleDelete(event: React.MouseEvent<HTMLDivElement>) {
        ...
        const [, error] = await deleteRecord({ record: recordToDelete, object: object });
                                        ...
                                        {Object.keys(columns).map((key) => (
                                            ...
                                            >

                                                {
                                                    ...
                                                    ) : columns[key] === "file" && item[key] ? (
                                                        <a href={'/api/services/file/' + (item[key] as Record<string, unknown>).fileName as string} target="_blank" rel="noopener noreferrer">
                                                            View File
                                                        </a>
                                                    ) : columns[key] === "text" && typeof item[key] === "string" && item[key].length > 50 ? (
                                                        item[key].slice(0, 50) + "..."
                                                    ) : (
